# shamelessly taken from Inflect which shamelessly took it from Zombie
...
task "install", "Install priority in your local repository", ->
  build (err) ->
    onerror err
    generateMan (err) ->
      onerror err
      log "Installing priority ...", green
...
#          html = html.replace(/<h1>(.*)<\/h1>/, "<h1>priority.js</h1><b>$1</b>")
...
documentSource = (callback) ->
  ...
  exec "docco src/priority/*.coffee", (err, stdout, stderr) ->
    log stdout, green
    onerror err
    callback()
